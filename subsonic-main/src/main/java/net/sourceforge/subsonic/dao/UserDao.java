...
public class UserDao extends AbstractDao {
    ...
    private static final String USER_COLUMNS = "username, password, email, ldap_authenticated, bytes_streamed, bytes_downloaded, bytes_uploaded, group_id";
    private static final String USER_SETTINGS_COLUMNS = "username, locale, theme_id, final_version_notification, beta_version_notification, " +
            ...
            "party_mode_enabled, now_playing_allowed, avatar_scheme, system_avatar_id, changed, show_chat, list_type, list_rows, list_columns, playqueue_resize, leftframe_resize, customscrollbar";
    ...
    public void createUser(User user) {
        ...
        update(sql, user.getUsername(), encrypt(user.getPassword()), user.getEmail(), user.isLdapAuthenticated(),
                user.getBytesStreamed(), user.getBytesDownloaded(), user.getBytesUploaded(), user.getGroupId());
        writeRoles(user);
    }
    ...
    public void updateUser(User user) {
        String sql = "update user set password=?, email=?, ldap_authenticated=?, bytes_streamed=?, bytes_downloaded=?, bytes_uploaded=?, group_id=? " +
                "where username=?";
        getJdbcTemplate().update(sql, new Object[]{encrypt(user.getPassword()), user.getEmail(), user.isLdapAuthenticated(),
                ...
                user.getGroupId(), user.getUsername() });
        writeRoles(user);
    }
    ...
    public void updateUserSettings(UserSettings settings) {
        ...
        getJdbcTemplate().update(sql, new Object[]{settings.getUsername(), locale, settings.getThemeId(),
                ...
                settings.isShowChatEnabled(), settings.getListType(), settings.getListRows(), settings.getListColumns(), 
                settings.getPlayQueueResize(), settings.getLeftFrameResize(), settings.isCustomScrollbarEnabled() });
    }
    ...
    private class UserRowMapper implements ParameterizedRowMapper<User> {
        public User mapRow(ResultSet rs, int rowNum) throws SQLException {
            User user = new User(rs.getString(1), decrypt(rs.getString(2)), rs.getString(3), rs.getBoolean(4),
                                 rs.getLong(5), rs.getLong(6), rs.getLong(7), rs.getInt(8));
            readRoles(user);
            return user;
        }
    }
