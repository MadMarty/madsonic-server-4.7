...
public class RESTController extends MultiActionController {
    ...
    public void getArtists(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        SortedMap<MusicIndex, SortedSet<MusicIndex.SortableArtistWithArtist>> indexedArtists = musicIndexService.getIndexedArtists(artists, 1); //TODO:hardcoded
...
//            }
    ...
    public void getAlbumList(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        if ("highest".equals(type)) {
            albums = homeController.getHighestRated(offset, size, userGroupId);
        } else if ("frequent".equals(type)) {
            albums = homeController.getMostFrequent(offset, size, userGroupId);
        } else if ("recent".equals(type)) {
            albums = homeController.getMostRecent(offset, size, userGroupId);
        } else if ("newest".equals(type)) {
            albums = homeController.getNewest(offset, size, userGroupId);
        } else if ("tip".equals(type)) {
            albums = homeController.getRandomHot(size, userGroupId);
        } else if ("hot".equals(type)) {
            albums = homeController.getHot(offset, size, userGroupId);
        } else if ("starred".equals(type)) {
            albums = homeController.getStarred(offset, size, username);
        } else if ("alphabeticalByArtist".equals(type)) {
            albums = homeController.getAlphabetical(offset, size, true, userGroupId);
        } else if ("alphabeticalByName".equals(type)) {
            albums = homeController.getAlphabetical(offset, size, false, userGroupId);
        } else if ("random".equals(type)) {
            albums = homeController.getRandom(size, userGroupId);
        } else {
...
//            attributes.add("albumSetName", mediaFile.getAlbumSetName());             
        }
        else {
...
//            attributes.add("album", mediaFile.getAlbumName());
//            attributes.add("albumSetName", parent.getAlbumSetName());             
        }
