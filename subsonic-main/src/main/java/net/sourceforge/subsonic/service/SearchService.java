...
public class SearchService {
    ...
    public List<MediaFile> getRandomSongs(GenreSearchCriteria criteria) {
        ...
        try {
        	...
        	if (criteria.getGenres() != null) {
            	for (String genre : criteria.getGenres()) {
            		if (genre != null) {
            			...
            			// String []genrePart = (genre.toString().split(" ")); 
            			// String genrePartFound = null ;
                        // try {
                        	// genrePartFound = genrePart[1];
                        // } catch (Exception x) {
                        // }
            			
            			// if (genrePartFound == null) {

            				// queryGenre.add(new TermQuery(new Term(FIELD_GENRE, genre.toLowerCase())), BooleanClause.Occur.SHOULD);
            			// }
            			// else
	            			// {
                			// for (String child : genrePart){
	                				// if ((child.toString().length()) > 2 ) {
	                					// queryGenre.add(new TermQuery(new Term(FIELD_GENRE, child.toLowerCase())), BooleanClause.Occur.SHOULD);
	                			    // }
	                			// }
		                	// }
        	...
        	queryMaster.add(queryFolder, BooleanClause.Occur.SHOULD);
     ...
     */
    public List<MediaFile> getRandomAlbums(int count, int user_group_id) {
        List<MediaFile> result = new ArrayList<MediaFile>();
        ...
        try {
            ...
            for (int i = 0; i < Math.min(count, topDocs.totalHits); i++) {
                ...
                int id = Integer.valueOf(doc.get(FIELD_ID));
                try {
                    addIfNotNull(mediaFileService.getMediaFile(id, user_group_id), result);
                } catch (Exception x) {
